...
uint64_t kernel_pgd;   // 进程默认用的内核页表根（注意：这是 VA 还是 PA 看你怎么用）
//add jimmy
...
void load_user_embedded(uint64_t addr)
{
    uint64_t size =
        _binary_init_bin_end - _binary_init_bin_start;

    memcpy((void*)addr, _binary_init_bin_start, size);
}
